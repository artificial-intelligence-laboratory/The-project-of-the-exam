...
<script>
	export default {
		data() {
			...
			return {
				index: 0,
				list: ['全部', '已接单', '完成中', '待验收','已完成'],		// index:0全部，1已接单，2完成中，3待验收，4已完成
				itemList0:[],	// 已接单的订单
				itemList1:[],	// 完成中的订单
				itemList2:[],	// 待验收的订单
				itemList3:[],	// 已完成的订单
		...
		methods: {
			...
			getMyOrder(){
				...
				// 获取状态0的
				uni.request({
					url: that.$baseUrl + '/order/getOrderByMe?isIssue=false&count='+this.Listcount+'&cur='+this.currentPage+'&status=0',
					method:"get",
					header:{
				...
				// 获取状态1的
				uni.request({
					url: that.$baseUrl + '/order/getOrderByMe?isIssue=false&count='+this.Listcount+'&cur='+this.currentPage+'&status=1',
					method:"get",
					header:{
						'Content-Type': 'application/json',
						'Authorization': uni.getStorageSync('token')
				...
				// 获取状态2的
				uni.request({
					url: that.$baseUrl + '/order/getOrderByMe?isIssue=false&count='+this.Listcount+'&cur='+this.currentPage+'&status=2',
					method:"get",
					header:{
						'Content-Type': 'application/json',
						'Authorization': uni.getStorageSync('token')
				...
				// 获取状态3的
				uni.request({
					url: that.$baseUrl + '/order/getOrderByMe?isIssue=false&count='+this.Listcount+'&cur='+this.currentPage+'&status=3',
					method:"get",
					header:{
						'Content-Type': 'application/json',
						'Authorization': uni.getStorageSync('token')
